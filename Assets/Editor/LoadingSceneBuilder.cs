...
{
    const string ScenePath = "Assets/Scenes/6_LoadingScene.unity";
    ...
    const string DefaultTargetSceneName = "3_PPE_Room";
    ...
    public static void Build()
    {
        ...
        if (logo == null)
            throw new InvalidOperationException("6_LoadingScene is missing TitleLogo2d.");
        ...
        ValidateScene(scene);
        Debug.Log("6_LoadingScene progress UI built: real async percentage and an authored 18-segment gradient bar are connected.");
    }
    ...
    static void ValidateScene(Scene scene)
    {
        ...
        if (splash != null)
            failures.Add("TitleSplashController must not remain in 6_LoadingScene because its Awake hides the logo.");
        if (sceneCamera == null
        ...
        if (failures.Count > 0)
        {
            string message = "6_LoadingScene progress UI validation failed:\n- " + string.Join("\n- ", failures);
            Debug.LogError(message);
            throw new InvalidOperationException(message);
        }

        Debug.Log("6_LoadingScene progress UI validation passed: real loader, 0% text, 18 gradient segments, and build target are valid.");
    }
